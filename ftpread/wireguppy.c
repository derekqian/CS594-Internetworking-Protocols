...
int main(int argc, char **argv) {
  ...
  while (1) {
	...
	if(phead.packetsize >= sizeof(ehead)+ipv4head.datalen) {
	  ...
	  dump((ipv4head.datalen-sizeof(ipv4head)-sizeof(tcphead))-(sizeof(ehead)+ipv4head.datalen-phead.packetsize), 2);
	}
      } else {
